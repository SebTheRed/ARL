...
export const CooldownProvider = ({ children }: any) => {
  ...
  const fetchCooldowns = async () => {
    ...
    try {
        Object.keys(gameRules.experiencesList).forEach((skill: any, i: number) => {
            Object.keys(gameRules.experiencesList[skill]).forEach((titleKey: any, i: number) => {
              const experienceTitle = gameRules.experiencesList[skill][titleKey].title;
              titleToKeyMapping[experienceTitle] = titleKey;
                const q = query(
                    collection(db,`users/${uid}/xpLog`),
                    where('eventTitle',"==",gameRules.experiencesList[skill][titleKey].title),
        ...
        querySnapshots.forEach((querySnapshot:any) => {
            if (!querySnapshot.empty) {
                ...
                if (experienceKey) {
                        ...
                        const experienceCooldown = gameRules.experiencesList[type][experienceKey].cooldown * 60 * 60 * 1000;
    ...
    return (
        <CooldownContext.Provider value={{ refreshCooldowns,cooldowns, cooldownsLoading }}>
            {children}
